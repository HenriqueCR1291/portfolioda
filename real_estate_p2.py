...
def get_links (url_realtor_br, pag):
    ...
    page_numbers = list(range(pag+101))[101:pag+101]
...
BRLR$""",'').replace('  ','')
    ...
    name_df = 'df_houses p2 - ' + str(date_time) + '.csv'
    ...
    if len(not_open) != 0:
        ...
        df_nt_open.to_csv("gs://archive_real_estate/csv/df_sites_not _open_p2.csv", index = None, header = True)
